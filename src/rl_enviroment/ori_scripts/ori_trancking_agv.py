...
class DoubleEscapeEnv(object):
    ...
    def __init__(self,parms_path):
        ...
        self.behivor = [0,1,2,3,4]  # 0 xÖ±ÐÐ£¬1 yÖ±ÐÐ£¬2 Ð±ÐÐ£¬ 3 Ðý×ª£¬4 »ìºÏ
    ...
    def step(self, action_1):
        ...
        time_noise = np.max([np.random.normal(0,self.time_noise),0])
    ...
    def pub_robot2_cmd(self):
        ...
        if self.robot2_cmd_pub_num % 10 == 0:
            ...
            if b == 0:
                ...
                # print(self.robot2_cmd_pub_num,'xÖ±ÐÐ')
            elif b == 1:
                self.cmd.linear.x = 0
                self.cmd.linear.y = y
                self.cmd.angular.z = 0
                # print(self.robot2_cmd_pub_num,'yÖ±ÐÐ')
            elif b == 2:
                self.cmd.linear.x = x
                self.cmd.linear.y = y
                self.cmd.angular.z = 0
                # print(self.robot2_cmd_pub_num,'Ð±ÐÐ')
            elif b == 3:
                self.cmd.linear.x = 0
                self.cmd.linear.y = 0
                self.cmd.angular.z = z
                # print(self.robot2_cmd_pub_num,'Ðý×ª')
            else:
                self.cmd.linear.x = x
                self.cmd.linear.y = y
                self.cmd.angular.z = z
                # print(self.robot2_cmd_pub_num,'»ìºÏ')
            self.robot2_cmd_pub_num = 0
        # else:
        #     if b == 0:
        #         print(self.robot2_cmd_pub_num,'xÖ±ÐÐ')
