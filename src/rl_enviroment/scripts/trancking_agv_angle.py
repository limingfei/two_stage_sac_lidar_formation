...
class DoubleEscapeEnv(object):
    ...
    def __init__(self,parms_path):
        ...
        self.exp_y_distance = self.parms['exp_y_distance']
        self.y_max_distance_err = self.y_max_distance - self.exp_y_distance
        self.y_min_distance = self.parms['y_min_distance']
    ...
    def _get_done(self,current_x_distance,current_y_distance,current_angle):
        if (current_x_distance > self.x_max_distance) or (current_y_distance > self.y_max_distance):
            done = True
            self.info['status'] = 0 # out of max distance
        elif (current_x_distance < self.x_min_distance) or (current_y_distance < self.y_min_distance):
            done = True
    ...
    def reward_distance(self, current_x_distance,current_y_distance, direction_error, dis_exp=None):
        ...
        current_y_distance_err = abs(self.exp_y_distance-current_y_distance)
    ...
    def _map_obs(self,orig_obs):
        ...
        y_err = orig_obs['pose']['y'] - self.exp_y_distance
