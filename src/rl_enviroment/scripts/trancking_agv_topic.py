...
class DoubleEscapeEnv(object):
    ...
    def step(self, action_1):
        ...
        # dt_time = rospy.Time.now()-self.time
        # self.time = rospy.Time.now()
        # print('dt_time',dt_time.to_sec(),'obs:',obs.shape)
        # self.rate.sleep()
    ...
    def _get_observation(self):
        ...
        # msg = rospy.wait_for_message("/robot1_cmd_vel",Twist)
        rospy.Subscriber('/robot1_odom',Odometry,self.callback)
        # trans = self.buffer.lookup_transform('robot1_base_link', 'robot2_base_link', rospy.Time(0),rospy.Duration(3))
        self.client.wait_for_service()
        relative_response = self.client.call(self.get_relative_requset)
        self.rate.sleep()
        if relative_response.success:
    ...
    def callback(self,msg):
        self.observation['robot1']['v_x'] = msg.twist.twist.linear.x
        self.observation['robot1']['v_y'] = msg.twist.twist.linear.y
        self.observation['robot1']['v_ang'] = msg.twist.twist.angular.z   
