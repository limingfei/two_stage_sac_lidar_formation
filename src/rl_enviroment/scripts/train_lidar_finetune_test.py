        ...
        while True:
            ...
            if args.test:
                if step == env.max_step:
                    np.savetxt("./leader_trajectory/trajectory_rate_{}_r_{}_n_{}.csv".format(env.pub_leader_rate,int(episode_reward),len(os.listdir('./leader_trajectory'))), env.leader_cmd, delimiter=",")
                    break
            else:
                if done:
                    break
...
def main():
    ...
    if args.test:
        ...
        args.log_path = './logs_test_finetune_room'
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--log_path',default='./logs_actor_critic_finetune_room_prevent_chan_reward',type=str
    )
    parser.add_argument(
