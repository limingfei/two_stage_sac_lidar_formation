...
class SAC(parl.Algorithm):
    ...
    def predict(self, src,tgt,vel,obs,student_mode=False):
        if student_mode:
            act_mean, _ = self.student_model.policy(src,tgt,vel)
        else:
    ...
    def student_sample(self,src,tgt,vel):
        act_mean, act_log_std = self.student_model.policy(src,tgt,vel)
        normal = Normal(act_mean, act_log_std.exp())
        # for reparameterization trick  (mean + std*N(0,1))
        x_t = normal.rsample()
        action = torch.tanh(x_t)
