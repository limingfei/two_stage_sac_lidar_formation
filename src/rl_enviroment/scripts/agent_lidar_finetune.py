...
class GazeboAgent(parl.Agent):
    ...
    def predict(self,src,tgt,vel):
        ...
        # obs = [obs0,obs1]
        action= self.alg.predict(src,tgt,vel)
    ...
    def sample(self,src,tgt,vel):
        ...
        vel = torch.tensor(vel.reshape(1,-1),dtype=torch.float32).to(self.device)
        # obs = torch.tensor(obs.reshape(1,-1),dtype=torch.float32).to(self.device)
        action,_ = self.alg.sample(src,tgt,vel)
        action_numpy = action.cpu().detach().numpy().flatten()
        return action_numpy
    ...
    def learn(self,src,tgt,vel,obs,action,expert_act,reward,next_src,next_vel,next_obs,terminal):
        ...
        critic_loss, actor_loss,alpha_loss,bc_loss,actor_reg_loss,critic_reg_loss = self.alg.learn(src,tgt,vel,obs,action,expert_act,reward,next_src,next_vel,next_obs,terminal)
